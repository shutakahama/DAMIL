...
class PretrainDAMIL:
    """Model Pre-traning.
    Train the instance classifier with the source dataset,
    and train the bag classifier with the target dataset.
    ...
    def train(self):
        ...
        for epoch in range(self.args.num_epochs_pre):
            ...
            for step, (source_batch, target_batch) in data_zip:
                ...
                pred_cls_list = np.append(
                    pred_cls_list, np.array(F.softmax(output_cls.data.cpu(), dim=1)), axis=0)
                pred_bag_list = np.append(
                    pred_bag_list, np.array(F.softmax(output_bag.data.cpu(), dim=1)), axis=0)
                pred_ins_list = np.append(
                    pred_ins_list, np.array(F.softmax(output_att.data.cpu(), dim=1)), axis=0)
                gt_cls_list = np.append(gt_cls_list, np.array(source_instance_label.data.cpu()), axis=0)
                gt_bag_list = np.append(
                    gt_bag_list, np.max(target_instance_label.data.cpu().numpy(), keepdims=True), axis=0)
    ...
    def predict(self, epoch, data_loader, split_type, data_category):
        ...
        with torch.no_grad():
            for step, batch in enumerate(data_loader):
                ...
                # Caluculate socres
                # If data_category = source, only scores for the instance classifier is recorded,
                # If data_category = target, scores for instance and bag classifiers are recorded.
                loss_cls += criterion(pred_cls, instance_label).data
                pred_cls_list = np.append(
                    pred_cls_list, np.array(F.softmax(pred_cls.data.cpu(), dim=1)), axis=0)
                ...
                if data_category == "target":
                    ...
                    pred_bag_list = np.append(
                        pred_bag_list, np.array(F.softmax(pred_bag.data.cpu(), dim=1)), axis=0)
                    pred_ins_list = np.append(
                        pred_ins_list, np.array(F.softmax(pred_att.data.cpu(), dim=1)), axis=0)
                    gt_bag_list = np.append(
                        gt_bag_list, np.max(instance_label.data.cpu().numpy(), keepdims=True), axis=0)
